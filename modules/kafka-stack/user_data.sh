...
mkdir -p build/ && \
aws s3 cp ${S3_KAFKA_STACK_PATH} build/ && \
unzip build/kafka-stack.zip && \
(cd kafka-stack/ && export POSTGRES_USER=postgres && export POSTGRES_PASSWORD=${POSTGRES_PASSWORD} && docker-compose up -d --build)
